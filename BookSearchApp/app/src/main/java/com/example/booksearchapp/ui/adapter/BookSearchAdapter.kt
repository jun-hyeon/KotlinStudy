...
class BookSearchAdapter : ListAdapter<Book, BookSearchViewHolder>(BookDiffCallback){
    ...
    override fun onBindViewHolder(holder: BookSearchViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
           onItemClickListener?.let { it(holder.itemView, position, book) }
//            itemClickListener.onClick(it, position, book)
        }
    }
    //강의에서 사용한 클릭 리스너
    private var onItemClickListener : ((view: View, position: Int,book: Book) -> Unit?)? = null
    fun setOnItemClickListener(listener : (view: View, position: Int,book: Book) -> Unit){
    ...
    interface OnItemClickListener{        //인터페이스
        fun onClick(v : View, position: Int, book: Book)
    }
